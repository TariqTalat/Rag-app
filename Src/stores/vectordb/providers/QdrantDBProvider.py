from qdrant_client import models, QdranctClient
...
class QdrantDBProvider(VectorDBInterface):
    ...
    def connect(self):
        self.client = QdranctClient(path = self.db_path)
    ...
    def insert_one(self, collection_name: str, text: str, vector: list,
                   ...
                   record_id: str= None):
        if not self.is_collection_existed(collection_name):
            self.logger.error(f"Can't insert new record to non-existing collection: {collection_name}")
            return False
